...
it('shows at most six measured cards and opens at the start of the scrollable queue', () => {
  ...
  expect(stack.style.getPropertyValue('--chat-queue-viewport-height')).toBe('502px');
  rerender(view(items));
  expect(stack.style.getPropertyValue('--chat-queue-viewport-height')).toBe('502px');
  expect(stack.style.getPropertyValue('--chat-queue-column-height')).toBe('670px');
...
it('fits the available space on short windows and restores the six-card limit on resize', () => {
  let bottom = 240;
  ...
  expect(stack.style.getPropertyValue('--chat-queue-viewport-height')).toBe('236px');
  const viewport = stack.firstElementChild as HTMLElement;
  viewport.scrollTop = 70;
  bottom = 1000;
  fireEvent.resize(window);
  expect(stack.style.getPropertyValue('--chat-queue-viewport-height')).toBe('322px');
